...
class Discriminator(nn.Module):
    def __init__(self, config):
        ...
        for it in range(config.blocks):
            mult = 2**it
            layers += (
                SpectralNorm(nn.Conv2d(config.d_dim*mult,  config.d_dim*mult*2, 5, stride=2, padding=2, bias=False)),
        ...
        prd_src_layers = (SpectralNorm(nn.Conv2d(config.d_dim * mult, 1, 4)), Reshape(-1, 1),)
        ...
        if self.auxclas:
            ...
            for it in range(self.numcats):
                self.predict_class += nn.Sequential(
                    SpectralNorm(nn.Conv2d(config.d_dim * mult, config.categories[it], 4)),
    ...
    def first_layers(self, config):
        return nn.Sequential(
            SpectralNorm(nn.Conv2d(config.imgch, config.d_dim, 5, stride=2, padding=2)),
            Activation(config.d_act)
        )
