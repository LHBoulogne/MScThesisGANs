    ...
    def weight_init(self,  method='xavier', mean=0.0, std=0.02):
        for m in self._modules:
            if method == 'normal':
                ...
                xavier_weights_init(self._modules[m])
...
class Discriminator(nn.Module):
    def __init__(self, d, imgch, categories=0):
        ...
        self.prelu2 = nn.PReLU()
        self.conv3 = nn.Conv2d(50*d, 2, kernel_size=1, stride=1, padding=0)
...
class CoGAN():
    ...
    def __init__(self, categories=0, z_len=100, g_d=128, d_d=128, imgch=3, loadfolder=None, weight_init_method='xavier'):
        ...
        self.G.weight_init(method=weight_init_method)
        self.D.weight_init(method=weight_init_method)
    ...
    def train(self,  mini_batch_size=128, k=1, nr_epochs=20, vis_step=10, vis_dim=6, savename="", weight_decay=0):
        ...
        y_real = torch.LongTensor(np.ones(mini_batch_size*2, dtype=np.int))#.mul_(0.9)
        y_real_v = Variable(y_real)
        y_fake = torch.LongTensor(np.zeros(mini_batch_size*2, dtype=np.int))
        ...
        s_criterion = nn.functional.cross_entropy
        c_criterion = nn.CrossEntropyLoss() #Includes the softmax function
        d_opt = optim.Adam(self.D.parameters(), lr=0.0002, betas=(0.5, 0.999), weight_decay=weight_decay)
        g_opt = optim.Adam(self.G.parameters(), lr=0.0002, betas=(0.5, 0.999), weight_decay=weight_decay)
        ...
        for epoch in range(nr_epochs):
            for batch, data in enumerate(dataloader) :
                ...
                error_D_real = s_criterion(verdict_real.view(-1, 2), y_real_v)  #reshape verdict: [?,1] -> [?]
                ...
                error_D_fake = s_criterion(verdict_fake.view(-1, 2), y_fake_v)
                if self.categories: # add loss for classc_criterion() prediction
                ...
                if steps_without_G_update == k: #Allow for multiple updates of D with respect to G
                    ...
                    error_G = s_criterion(verdict_fake.view(-1, 2), y_real_v) #use real labels
                    if self.categories:
...
g_d = int(ar.next_arg())
d_d = int(ar.next_arg())
weight_init_method = ar.next_arg()
weight_decay = float(ar.next_arg())
mini_batch_size = int(ar.next_arg())

cogan = CoGAN(0, imgch=1, g_d=g_d, d_d=d_d, weight_init_method=weight_init_method)
cogan.train(mini_batch_size=mini_batch_size, weight_decay=weight_decay, savename=ar.arg_string)
